...
def test_config_send(otg_api: snappi.Api, b2b_config: snappi.Config):
    config_to_send_dict = b2b_config.serialize(encoding=snappi.Config.DICT)

    otg_api.set_config(b2b_config)

    config: snappi.Config = otg_api.get_config()
...
def test_port_metrics(otg_api: snappi.Api, b2b_config: snappi.Config):
    otg_api.set_config(b2b_config)

    ts = otg_api.control_state()
    ts.traffic.flow_transmit.state = ts.traffic.flow_transmit.START
    ts.traffic.flow_transmit.flow_names = ["f1"]
    warning: snappi.Warning = otg_api.set_control_state(ts)

    print(warning)
    assert len(warning.warnings) == 0

    metrics_req: snappi.MetricsRequest = otg_api.metrics_request()

    start = datetime.datetime.now()
    while True:
        metrics_resp: snappi.MetricsResponse = otg_api.get_metrics(metrics_req)
...
@pytest.mark.filterwarnings("ignore:.*Unverified HTTPS request.*:")
def test_flow_metrics(otg_api: snappi.Api, b2b_device_config: snappi.Config):
    otg_api.set_config(b2b_device_config)
